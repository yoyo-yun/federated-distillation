...
teacher_model_name = "ClinicalBERT"
# 构建数据集
epochs = 60
batch_size = 6
T = 3
...
center_model_name = "MiniLM_" + teacher_model_name
...
teacher_model = torch.load("./Euadr_model/center_model_ClinicalBERT.pkl", map_location='cuda:0')
...
res_train = pd.concat([df1, df2, df3, df4, df5, df6, df7], axis=0, ignore_index=True)
labels = res_train['label'].tolist()
text_values = res_train['sentence'].tolist()
...
# loadData val
pd1 = pd.read_csv("./shuffleDatasets/data_8.csv")
pd2 = pd.read_csv("./shuffleDatasets/data_9.csv")
res_test = pd.concat([pd1,pd2], axis=0, ignore_index=True)
labels_list_val = res_test['label'].tolist()
text_values_val = res_test['sentence'].tolist()
...
for epoch in range(epochs):
    ...
    for step, batch in tqdm(enumerate(train_dataloader)):
        ...
        outputs_teacher = teacher_model(batch[0].to(device), token_type_ids=None,
    ...
    for i, batch in enumerate(val_dataloader):
        with torch.no_grad():
            ...
            loss = F.cross_entropy(
                logits.view(-1, 2), batch[1].squeeze(-1).to('cuda'),
                reduction='mean')
            # loss = output['loss']
            # logits = output['logits']
