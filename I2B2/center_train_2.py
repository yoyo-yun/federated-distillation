...
model_name = "oliverqq/scibert-uncased-topics"
...
model.classifier = nn.Sequential(nn.Linear(768,246),nn.Linear(246,5),nn.Softmax())
...
df0 = pd.read_csv("./i2b2.csv")[:train_len]
...
# loadData val
train_len=int(7133*0.8)
pd1 = pd.read_csv("./i2b2.csv")[train_len:]
...
class MultilabelTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=True):
        ...
        loss = F.cross_entropy(
            logits.view(-1, 5), labels.squeeze(-1),
            reduction='mean')
