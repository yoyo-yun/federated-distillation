...
teacher_model_6 = torch.load("./clients_model/clients_model_ClinicalBERT.pkl", map_location=teacher_device)
teacher_model_list = [teacher_model_1, teacher_model_2, teacher_model_5,
                      teacher_model_6, teacher_model_4]
...
res = pd.concat([df8], axis=0, ignore_index=True)
...
def flat_accuracy(preds, labels):
    ...
    return accuracy_score(labels_flat, pred_flat), precision_score(labels_flat, pred_flat, average="micro"), \
           recall_score(labels_flat, pred_flat, average="micro"), f1_score(labels_flat, pred_flat, average="micro")
...
class MultilabelTrainer(Trainer):
    def compute_loss(self, model, inputs, logits_teacher, vid_loss, return_outputs=True):
        ...
        loss_nll = F.cross_entropy(
            logits.view(-1, 9), labels.squeeze(-1),
            reduction='mean')

        # logits = F.softmax(logits)
        # logits_teacher = F.softmax(logits_teacher)
        d_loss = nn.KLDivLoss(reduction='mean')(F.log_softmax(logits.view(-1, 9) / T, dim=1),
                                                F.softmax(logits_teacher.view(-1, 9) / T,
                                                          dim=1)) * T * T
        d_loss = d_loss.mean()
        vid_loss = vid_loss(F.softmax(logits.view(-1, 9), dim=1),
                            F.softmax(logits_teacher.view(-1, 9), dim=1), 'mean')
...
def vote(logits_list):
    ...
    for i, temp_logits in enumerate(logits_list):
        ...
        arg_logits = torch.nn.functional.one_hot(arg_logits, 9)
        # print(arg_logits)
    ...
    for i, batch in enumerate(val_dataloader):
        with torch.no_grad():
            ...
            loss = F.cross_entropy(
                logits.view(-1, 9), batch[1].squeeze(-1).to(cuda_index),
                reduction='mean')
            # loss = output['loss']
            # logits = output['logits']
