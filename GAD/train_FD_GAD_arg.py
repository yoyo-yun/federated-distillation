...
teacher_model0 = torch.load("./clients_model/model3_bio_bert.pkl").to(device)
...
class MultilabelTrainer(Trainer):
    def compute_loss(self, model, inputs, logits_teacher, vid_loss, return_outputs=True):
        ...
        loss_all = (alpha) * loss_nll + (1 - alpha) * d_loss + 0.2 * vid_loss
...
for epoch in range(epochs):
    ...
    for step, batch in tqdm(enumerate(train_dataloader)):
        ...
        for i in range(clients_num):
            ...
            teacher_model0.load_state_dict(
                torch.load("./clients_model_arg/model" + str(rand_num) + "_bio_bert.pth", map_location='cuda:0'))
            outputs_teacher0 = teacher_model0(batch[0].to(device), token_type_ids=None,
                                              attention_mask=(batch[0] > 0).to(device))
            logits_teacher0 = outputs_teacher0.get('logits')
            logits_teacher0 = F.one_hot(torch.argmax(logits_teacher0, dim=-1))
            teacher_logits_list.append(logits_teacher0)
        x = torch.zeros_like(teacher_logits_list[0])
        for j in teacher_logits_list:
            x += j
        logits_teacher = x / clients_num
    ...
    for i, batch in enumerate(val_dataloader):
        with torch.no_grad():
            model = model.to('cpu')
            output = model(batch[0], token_type_ids=None, attention_mask=(batch[0] > 0),
                           labels=batch[1])
